...
We will get our hands dirty and code everything! In this tutorial, we will use Python3 and scipy but I hope that the code and the ideas will be clear enough so that you can adapt the code to your favourite language. 
...
Then, we will tackle recurrent neural networks and show how to model them with our library. To apply our new knowledge, we will try to learn a grammar generated by an automaton.
...
It’s time for another application, don’t you think? Have you ever read [this fabulous article](http://karpathy.github.io/2015/05/21/rnn-effectiveness/) by Andrej Karpathy? Yes? Cool, because, we are going to reproduce his results with our own library. Amazing, isn’t it?

Finally, part 6 and 7 are going to be theoretical appendices for the most curious readers.
...
Another reason why I will not present neural networks, is that we will adopt a modern approach on neural networks. We will not see a neural networks as a network of neurons but instead we will consider them as parametric functions.
...
We still have a big problem. What is $$f$$? We could try to find the best $$f$$ in the whole space function but the problem is intractable and it is not desirable because there are very poor solutions which have a cost equal to zero. We will instead restrict ourselves to a specific class of functions, like I said before, the parametric functions. A parametric function is simply a function $$f_{\theta}$$ that depends on a parameter $$\theta$$.
...
I am going to present quickly an algorithm to find a good $$\theta$$. The algorithm is called *gradient descent*. It consists of choosing randomly an initial parameter $$\theta_0$$ and at each step of the algorithm we will optimize locally to improve the solution. The idea is to make small steps to diminish the cost. Formally, let $$\theta_t$$ be the value of the parameter at iteration $$t$$ of the algorithm. At iteration $$t$$, the direction toward which the cost is decreasing the most is $$-\frac{\partial J}{\partial \theta}(\theta_t)$$. Consequently, we will make a small step in this direction :

$$
\theta_{t+1} = \theta_t - \eta \frac{\partial J}{\partial \theta}(\theta_t)
$$

where $$\eta$$ is called the *learning rate*. It's a parameter that controls the size of the steps we make. We will dig deeper into optimization algorithms later in another part.
...
The following computational graphs corresponds to the neural network depicted above.
...
There are three main classes that we are goind to code in this part : the Node class, the Graph class and the OptimizationAlgorithm class.

The Node class contains most of the logic of the library. It's the hard part.

The Graph class is a container that contains nodes and allow to interact easily with them.
...
In the next subsections, we are going to see the specialization of this class.
...
The class `ParameterNode` has to new parameters `w` and `acc_dJdw`. We use the letter `w` to refer to the weights, like in neural networks. `acc_dJdw` represents an accumulator of the derivative of the cost with respect to the weights `w` over several backpropagation.
...
If you want to see all the formulas and how to derive the backpropagation formulas you can go to the appendice on matrix derivations.
...
It takes the list of all the nodes in the graph. In addition, it needs to know the function of several nodes in the graph : the inputs, the ouputs, the expected outputs which can be useful to compute the cost and the cost.
...
We will use the function `fully_connected` which takes as input a list of the sizes of the layers and returns a computational graph.
...
* Try to create other datasets using other functions and learn them.